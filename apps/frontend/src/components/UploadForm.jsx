...
export default function UploadForm({ onGenerated, setIsGenerating }) {
  ...
  const handleGenerateChapter = async (chapterId) => {
    ...
    try {
      ...
      setProgress({
        step: 'generating',
        message: `Generating flashcards for: ${chapter.title}...`,
      });
      const result = await api.generateChapter(uploadId, examName, chapterId, maxCards);
    ...
    return (
      <div className='bg-white p-6 rounded-lg shadow-md max-w-2xl mx-auto'>
        <div className='flex justify-between items-center mb-4'>
          ...
          <button onClick={handleStartOver} className='text-sm text-gray-500 hover:text-gray-700'>
            Start Over
          </button>
        </div>
        ...
        {currentChapter && (
          <div className='bg-green-50 border border-green-200 rounded-lg p-4 mb-4'>
            ...
            <p className='text-sm text-green-700 mb-3'>
              {allFlashcards.filter((c) => c.topic === currentChapter.title).length} flashcards
              created
            </p>
            <button
        ...
        <div className='mb-4'>
          <p className='text-sm text-gray-600 mb-2'>
            Total flashcards: <strong>{allFlashcards.length}</strong> | Chapters completed:{' '}
            <strong>{generatedChapterIds.size}</strong>/<strong>{chapters.length}</strong>
          </p>
        </div>
  ...
  return (
    <div className='bg-white p-6 rounded-lg shadow-md max-w-md mx-auto'>
      ...
      <form onSubmit={handleUpload}>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (e.target.files?.[0]) {
                log(
                  'FILE_SELECTED',
                  `${e.target.files[0].name} (${(e.target.files[0].size / 1024 / 1024).toFixed(2)} MB)`,
                );
              }
            }}
        ...
        <div className='mb-4'>
          <label className='block text-sm font-medium text-gray-700 mb-1'>Exam Name / Code</label>
          <input
            type='text'
            value={examName}
